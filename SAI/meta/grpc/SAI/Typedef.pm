...
sub grpc_def {
    ...
    my $name = $self->type->grpc_name( $self->raw );
...
sub grpc_def_gpb {
    ...
    # with string - only for ip/mac addresses
    $name = 'string' if $self->name =~ /(ip\d+|mac)_t$/;
    $name = 'int64' if $name eq 'i64';
    $name = 'int32' if $name eq 'i32';
    $name = 'int32' if $name eq 'i16';
    $name = 'int32' if $name eq 'i8';
    $name = 'repeated' if $name eq 'list';
