...
around grpc_name => sub {
    ...
    if ( $self->is_attr_list ) {
        return 'sai_grpc_attribute_list_t';
    }
    else {
        return '' . $self->subtype->grpc_name . ' '
          if $self->is_list
    ...
    # If $raw then convert the type to grpc type
...
sub cpp_name {
    ...
    {
        return 'std::vector<' . $self->subtype->grpc_name . '>';
    }

    return 'std::string' if $self->name =~ /void/;

    return $self->grpc_name( 0, $name );
}
...
sub python_name {
    ...
    if (    $self->is_list
        ...
        and not $self->is_attr_list )
    {
        return 'List[' . $self->subtype->grpc_name . ']';
    }
