...
use lib catdir( dirname( rel2abs(__FILE__) ), 'grpc' );
...
const my $PREFIX         => 'sai_grpc_';
const my $GEN_DIR        => 'generated_grpc';
const my $GRPC_GEN_DIR => 'gen-cpp';
const my $COUNTER        => -1;
const my $RETVAL         => 0;
...
say colored( 'Generating sai.grpc...', 'bold blue' );
$template->process( catfile( $templates_dir, 'sai.grpc.tt' ),
    $vars, catfile( $run_dir, 'sai.proto' ) )
  or die $template->error();

say colored( 'Generating gRPC files...', 'bold blue' );
chdir $run_dir;
system('protoc', '--c_out=' . $gen_dir, 'sai.proto');

chdir $run_dir;

say colored( "files generated in $gen_dir...", 'bold blue' );
#say colored( 'Generating sai_rpc_server.cpp.tt...', 'bold blue' );
#open my $skeleton, '<',
#  catfile( $gen_dir, $GRPC_GEN_DIR, 'sai_rpc_server.skeleton.cpp' )
#  or die colored( 'Thrift files not generated, probably sai.grpc is invalid',
#    'red' );
...
sub get_attr_type {
    ...
    for ( @{ $attr_types->members } ) {
        return $_->grpc_name if ( $type eq $_->type->grpc_name );
    }
...
sub get_typedef {
    ...
    # If $raw, the we can use raw grpc types only
    $raw //= 0;
...
sub get_struct {
    ...
    $all_structs->{ $struct->grpc_name } = $struct;
